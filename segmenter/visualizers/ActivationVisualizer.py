...
def activations(clazz):
    ...
    results = {}
    aggregator_name, aggregator, fold_activation, final_activation, thresholds = get_aggregators(
        job_config)[0]
        ...
        for i, (m, outputs) in enumerate(all_outputs):
            for batch, (images, masks) in enumerate(dataset):
                print("{} model {} {}/{} image {}/{}".format(
                    layer_type, m.name, i + 1, len(all_outputs), batch + 1,
                    num_images))
    ...
    np.savez_compressed(
        os.path.join(outdir, job_hash, clazz, "results",
                     "activation_histogram"), **results)
