...
def find_latest_weight(folder):
    ...
    files = [os.path.join(folder, f) for f in os.listdir(
        folder) if os.path.isfile(os.path.join(folder, f)) if f.endswith(".h5")]
    if len(files) == 0:
        return None
...
def full_model(clazz, modeldir, job_config, job_hash, load_weights=True, fold_activation="linear", final_activation="sigmoid", aggregator=Average):
    ...
    boost_folds = range(
        job_config["BOOST_FOLDS"] + 1) if job_config["BOOST_FOLDS"] is not None else [None]
    ...
    for fold in folds:
        ...
        for boost_fold in boost_folds:
            ...
            if load_weights:
                best_weight = find_best_weight(os.path.join(
                    modeldir, job_hash, clazz, boost_fold_name))
            ...
            boost_fold_model = Lambda(lambda x: logit(
                x), name="{}_logit".format(boost_fold_name))(boost_fold_model)
        ...
        fold_model = Activation(fold_activation, name="{}_{}".format(
