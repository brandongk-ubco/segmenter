...
def custom_unet(
    ...
    for l in range(num_layers):
        x = conv2d_block(inputs=x, filters=filters, use_batch_norm=use_batch_norm, dropout=dropout, max_dropout=max_dropout, activation=activation, kernel_initializer=kernel_initializer)
        down_layers.append(x)
        x = MaxPooling2D((2, 2)) (x)
        dropout += dropout_change_per_layer
        filters = filters*filter_ratio # double the number of filters with each layer
    ...
    for conv in reversed(down_layers):
        filters //= filter_ratio # decreasing number of filters with each layer 
        dropout -= dropout_change_per_layer
        x = upsample(inputs=x, filters=filters, kernel_size=(2, 2), strides=(2, 2), padding='same', use_batch_norm=use_batch_norm, dropout=dropout, max_dropout=max_dropout, activation=activation, kernel_initializer=kernel_initializer)
